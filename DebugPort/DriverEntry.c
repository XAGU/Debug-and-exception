...
typedef NTSTATUS(NTAPI *KESETAFFINITYTHREAD)(
	IN PKTHREAD Thread,
...
BOOLEAN CreateMonitor(ULONG address, BOOLEAN remove)
{
	...
	ULONG				fnp_KeSetAffinityThread;
	UNICODE_STRING		usFuncName;
	RtlInitUnicodeString(&usFuncName, L"KeSetAffinityThread");
	fnp_KeSetAffinityThread = (ULONG)MmGetSystemRoutineAddress(&usFuncName);
	if (fnp_KeSetAffinityThread == 0)
	{
		return FALSE;
	}
	...
	while (Affinity)
	{
		...
		((KESETAFFINITYTHREAD)fnp_KeSetAffinityThread)(PsGetCurrentThread(), (KAFFINITY)CurrentAffinity);
		CurrentAffinity <<= 1;
		if (remove)
		{
			DelMonitor();
